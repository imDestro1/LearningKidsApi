...
namespace LearningKidsAPI.Services
{
    public class UsuarioService
    {
        ...
        private static readonly string[] PersonalRoleNames = ["docente", "administrador", "administradores", "admin"];
        ...
        public async Task<Usuario?> LoginPersonalAsync(string username, string password)
        {
            var cleanUsername = username.Trim();
            var cleanPassword = password.Trim();
            ...
            return await _context.Usuarios
                ...
                .Where(u => u.Rol != null
                    && u.Rol.nombre != null
                    && PersonalRoleNames.Contains(u.Rol.nombre.Trim().ToLower()))
                .FirstOrDefaultAsync();
        }
    }
}
